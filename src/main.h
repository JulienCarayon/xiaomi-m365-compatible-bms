...
/*2E-2F*/   uint16_t version = 0x115; // 0x115 = 1.1.5
/*30-31*/   uint16_t design_capacity = 7600; // mAh
/*32-33*/   uint16_t real_capacity = 7500; // mAh
/*34-35*/   uint16_t nominal_voltage = 36000; // mV
/*36-37*/   uint16_t num_cycles = 3;
/*38-39*/   uint16_t num_charged = 2;
/*3A-3B*/   uint16_t max_voltage = 4200; // V/100
/*3C-3D*/   uint16_t max_discharge_current = 3000; // A/100
/*3E-3F*/   uint16_t max_charge_current = 200; // A/100
/*  40-41  */   uint16_t date = (18 << 9) | (10 << 5) | 1;  // MSB (7 bits year, 4 bits month, 5 bits day) LSB
/*42-47*/   uint8_t errors[6] = {0};
/*48-5F*/   uint16_t unk3[12] = {0};
/*60-61*/   uint16_t status = 0xb0; // b0 = config valid, b6 = charging, b9 = overvoltage, b10 = overheat
/*62-63*/   uint16_t capacity_left = 1000; // mAh
/*64-65*/   uint16_t percent_left = 15;
/*66-67*/   int16_t current = 2; // A/100
/*68-69*/   uint16_t voltage = 3100; // V/100
/*6A-6B*/   uint8_t temperature[2] = {20, 20}; // °C - 20
/*6C-6D*/   uint16_t balance_bits = 0;
/*6E-75*/   uint16_t unk5[4] = {0};
/*76-77*/   uint16_t health = 100; // %, <60% = battery bad
/*78-7F*/   uint16_t unk6[4] = {0};
/*80-9D*/   uint16_t cell_voltages[15] = {3100,3100,3100,3100,3100,3100,3100,3100,3100,3100,0,0,0,0,0}; // mV
